...
\begin{mydefn}
 Sea una cadena de Markov $\{X_t\}_{t\ge 0}$ con espacio de estados $\mathbb{N}\cup \{0\}$, donde el espacio de estados representa el número de individuos
 de un sistema (población). Entonces $\{X_t\}_{t\ge 0}$ se dice proceso de nacimiento y muerte si existen tasas no negativas de nacimiento y muerte,
...
Llamando $P_n(t) = P[X_t = n]$ se verifica:
...
De hecho, también es condición suficiente. Si dicha distribución existe se tiene: \[p_0 = S^{-1}\]

En el caso de los sistemas de colas, si $S$ fuese convergente, quiere decir que la cola puede procesar todos los
eventos, mientras que si $S$ diverge, la cola no puede procesar todos los eventos, y el sistema de cola se dice
inestable (la frecuencia de llegadas es mayor en media que la frecuencia de salida).
%% Mas cosas
